...
export class CampaignController {
  ...
  async sendCampaign(
    ...
    return await this.campaignsService.sendCampaign(
      campaignId,
      user.email.split('@')[0],
    );
  }
  ...
  async handleSendEmail(
    @Payload() data: { username: string; to: string; text_content: string },
  ) {
    try {
      return await this.campaignsService.sendInvoiceEmail(
        data.username,
        data.to,
        data.text_content,
      );
